...
public class EmailSender
{
    ...
    public void PsdSend(string mailToAddress, string newPwd)
    {
        ...
        using (var smtpClient = new SmtpClient())
        {
            ...
            string body = "您在五金配件商城的密码已重置为：" + newPwd;
    ...
    public string VerificationCodeSend(string mailToAddress)
    {
        ...
        using (var smtpClient = new SmtpClient())
        {
            ...
            //设置SMTP服务器发送邮件的凭据（用户名和授权码)
            smtpClient.Credentials = new NetworkCredential(userName, password);
            string body = "你的验证码为：" + code + " ，仅用可于修改支付密码，请勿转发！若非本人操作，请忽略此邮件！";
            MailMessage mailMessage = new MailMessage(
                                   mailFromAddress,   // 发件人邮箱
                                   mailToAddress,     // 收件人邮箱
                                   "五金配件商城验证码",  // 电子邮件主题
    ...
    //注册验证码发送
    public string RegisterCodeSend(string mailToAddress)
    {
        //伪随机生成6位数字验证码
        Random rand = new Random();
        string code = rand.Next(100000, 999999).ToString();
        ...
        using (var smtpClient = new SmtpClient())
        {
            ...
            //设置SMTP服务器发送邮件的凭据（用户名和授权码)
            smtpClient.Credentials = new NetworkCredential(userName, password);
            string body = "你的注册验证码为：" + code + "。若非本人操作，请忽略此邮件！";
            MailMessage mailMessage = new MailMessage(
                                   mailFromAddress,   // 发件人邮箱
                                   mailToAddress,     // 收件人邮箱
                                   "五金配件商城验证码",  // 电子邮件主题
